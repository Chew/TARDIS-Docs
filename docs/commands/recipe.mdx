...
<CommandTable data={
    ...
    usage: /<command> [item|list]
    `
}
args={
    `
    list:
      description: Lists all TARDIS recipe items.
      usage: /<command> list
    `
}
extra='false'
...
| Sonic Blaster         | `/tardisrecipe sonic-blaster`       |
| Vortex Manipulator    | `/tardisrecipe vortex-manipulator`  |
...
| Conversion Circuit          | `/tardisrecipe conversion-circuit`          |
...
| Conversion Upgrade    | `/tardisrecipe conversion-upgrade`    |
...
| Exterior Lamp Level Switch  | `/tardisrecipe exterior-lamp`        |
| Interior Light Level Switch | `/tardisrecipe light-level`          |
