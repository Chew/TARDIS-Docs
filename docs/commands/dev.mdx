...
args={
    ...
    list:
      description: Lists various options to aid documentation - possible listings are \`preset_perms\`, \`perms\`, \`recipes\`, \`blueprints\`, \`commands\`, \`consoles\`, \`trades\`, \`block_colours\`.
...
args={
    ...
    block:
      description: Spawns an Item Display **door** block on top of the block the player is targeting. To specify which door state to place add a third argument - a number from \`1\` - \`10\` - e.g. \`/tdev displayitem block 4\`.
      usage: /<command> displayitem add
    remove:
