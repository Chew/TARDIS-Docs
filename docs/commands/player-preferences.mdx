...
Sonic Screwdriver, sneaking and right-clicking air or using the _Prefs_ button in the [Control Menu GUI](/control-menu) will open the [Player Preferences](/player-prefs-menu) GUI. Use the _General Preferences Menu_ button to access all options.

![Player prefs GUI](/images/docs/player_prefs_custom.jpg)

![Player prefs GUI](/images/docs/player_prefs_general.jpg)
...
args={
    ...
    build:
      description: Toggle companion building inside the TARDIS on and off. _Requires the WorldGuard plugin installed on the server_.
    ...
    dynamic_lamps:
      description: Toggle whether a player's preferred light type is used when growing rooms (instead of a default redstone lamp).
      permission: tardis.prefs.dynamic_lamps
      usage: /<command> dynamic_lamps [on|off]
    dnd:
    ...
    lights:
      ...
      usage: /<command> lights [light type]
    minecart:
...
args={
    ...
    wool_lights:
      description: Choose whether to use black wool or sponge (re-textured with the TARDIS Resource Pack to look like Redstone Lamp Off blocks) for the off state of the TARDIS lights.
