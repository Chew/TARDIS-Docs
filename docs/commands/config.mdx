...
args={
    ...
    reload:
      description: Reload the TARDIS config from disk. Where \`[config name]\` is one of \`achievements\`, \`adaptive\`, \`artron\`, \`blaster\`, \`blocks\`, \`chameleon_guis\`, \`condensables\`, \`custom_consoles\`, \`custom_desktops\`, \`flat_world\`, \`handles\`, \`items\`, \`kits\`, \`monsters\`, \`planets\`, \`recipes\`, \`rooms\`, \`shop\`, \`signs\`, \`tag\`, \`vortex_manipulator\`.
...
creation:
    ...
    seed_block:
        crafting: true # Sets whether players can craft TARDIS seed blocks.
        legacy: true # Sets whether server difficulty affects the 'lamp' crafting material. A value of \`true\` means the crafting material is always a Redstone Torch.
        easy: REDSTONE_TORCH
        normal: SOUL_LANTERN
        hard: DRAGON_BREATH # Sets the 'lamp' craft material depending on the **server difficulty**.
police_box:
    ...
    sign_colour: WHITE # Sets the colour of the text on a TARDIS sign. Must be a valid NamedTextColor value, see [https://javadoc.io/doc/net.kyori/adventure-api/4.3.0/net/kyori/adventure/text/format/NamedTextColor.html](https://javadoc.io/doc/net.kyori/adventure-api/4.3.0/net/kyori/adventure/text/format/NamedTextColor.html)
...
preferences:
    ...
    key: GOLD_NUGGET # Sets the TARDIS key item. You can use any valid Bukkit Material ENUM. A list can be found here: [https://jd.papermc.io/paper/26.2/org/bukkit/Material.html](https://jd.papermc.io/paper/26.1.2/org/bukkit/Material.html)
...
allow:
    ...
    hunger_and_healing: true # Sets whether the Kitchen and Surgery rooms automatically feed and heal players in the TARDIS from items placed in their respective room chests.
growth:
...
comehere: 400
hide: 500
autonomous: 100 # Sets the amount of energy the different forms of time travel consume. The options are: random locations, any \`/tardistravel\`, \`/tardis come_here\`, \`/tardis hide\` commands, and autonomous homing.
nether_min: 4250
