...
zero: 250
# upgrade costs
# since v6.3.11 upgrade costs are calculated
...
full_charge: 10000 # Sets the amount of energy a fully charged TARDIS Artron Energy Capacitor holds.
...
upgrades:
    ...
    cost_factor: 42.5 # Sets the percentage of \`full_charge\` the calculated upgrade costs use 
...
artron_furnace:
    ...
    tardis_powered: true # Whether you can power furnaces with TARDIS energy.
    power_drain: 200 # How much Artron is used per cycle.
    power_cycle: 400 # How often the furnace is fueled (in ticks).
    power_limit: 4 # Maximum number of powered furnaces per TARDIS.
sonic_generator:
    standard: 10
    bio: 10
    diamond: 10
    ...
    knockback: 10
    brush: 10
    conversion: 10 # Sets the energy costs for the [sonic generator](sonic-generator) as a percentage of the \`full_charge\` option.
