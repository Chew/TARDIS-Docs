...
creation:
    ...
    tips_next: HIGHEST # Sets whether T.I.P.S tries to reuse slots - \`FREE\`, or just selects the next highest numbered slot - \`HIGHEST\`.
    ...
    seed_block:
      ...
      easy: REDSTONE_TORCH # Sets the 'lamp' craft material when server difficulty is set to **peaceful** / **easy**.
      normal: SOUL_LANTERN # Sets the 'lamp' craft material when server difficulty is set to **normal**.
      hard: DRAGON_BREATH # Sets the 'lamp' craft material when server difficulty is set to **hard** / **hardcore**.
`} />
