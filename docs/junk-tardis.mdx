...
(this is the default option with new installs). In the config, set `creation.default_world: true`. You will also need
to enable the Junk TARDIS — `junk.enabled: true`.
...
There are eight controls that let you set a Junk TARDIS destination:
...
- **crimson button** - locates the player's TARDIS and sets the destination to a neighbouring chunk
