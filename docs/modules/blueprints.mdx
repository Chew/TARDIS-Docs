...
4. Trading with [travelling time lords](/travelling-time-lord).
...
villager_blueprints:
    ...
    chance: 20 # Sets the random percent chance that a villager will have blueprints to trade when it is spawned.
`} />

Each villager will have 4 trades - three rooms and one console. The purchase item will be the seed block that is used to grow the room or craft the console seed.

:::info

You can set the amount of the purchase item in _trades.yml_. For more information see the [Blueprint trades configuration options](/configuration/trades) page.
